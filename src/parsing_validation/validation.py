...
def validate_map(map: Map):
    if map.dimensions['rows'] * map.dimensions['columns'] != len(map.cells):
        raise ValidationError(
            f"Dimension is not apply to actual length of cells. Dimension {map.dimensions['rows']}x{map.dimensions['columns']}, but cells are {len(map.cells)}")

    if map.dimensions['rows'] <= 2 and map.dimensions['columns'] <= 2:
        raise ValidationError(
            f"Map size must be more than 2x2. Actual size is {map.dimensions['rows']}x{map.dimensions['columns']}")
